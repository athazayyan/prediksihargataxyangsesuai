...
def load_data():
    ...
    # Add harga carbon price Indonesia
    ...
    # Minyak mentah harga perthaunnya
    oil_price = pd.Series({2017: 51.31, 2018: 68.16, 2019: 65.50, 2020: 44.52, 2021: 70.65, 2022: 114.27})
    df['crude_oil_price'] = oil_price
    
    # investasi di sektor energi dengan perhitungan 1 dollar 14 ribu, dan semunya dalam bentuk milliar dolar
...
for var in variables:
    ax.plot(df.index, df[var], label=var, marker='1')  
...
for year in range(df.index.max() + 1, future_years + 1):
    ...
    future_X['population'] *= 1.01  # 0.88% annual population growth
